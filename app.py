...
# 세션스테이트 초기화
for _k, _v in [("kakao_candidates",[]), ("do_search",False), ("selected_apt_info",None),
               ("_pending_loc",""), ("_pending_apt","")]:
    ...
    if st.session_state.kakao_candidates:
        ...
        for c in st.session_state.kakao_candidates:
            ...
            if col_b.button("선택", key=f"sel_{c['아파트명']}"):
                # 위젯 렌더링 전에 적용하기 위해 임시 키에 저장 후 rerun
                st.session_state._pending_loc = _extract_loc(c['주소'])
                st.session_state._pending_apt = c['아파트명']
...
# rerun 직후 위젯 렌더링 전에 pending 값을 텍스트 입력 기본값으로 반영
_loc_default = st.session_state.pop("_pending_loc") if st.session_state.get("_pending_loc") else ""
_apt_default = st.session_state.pop("_pending_apt") if st.session_state.get("_pending_apt") else ""

col1, col2, col3, col4, col5 = st.columns([2, 2, 1, 1, 1])
with col1:
    location = st.text_input("소재지", value=_loc_default, placeholder="예: 대구 수성구 범어동")
with col2:
    apt_name = st.text_input("아파트명", value=_apt_default, placeholder="예: 범어자이")
with col3:
...
if auto_search:
    ...
    # auto_search 시 위젯값 대신 pending에서 가져온 값 사용
    if _loc_default:
        location = _loc_default
    if _apt_default:
        apt_name = _apt_default
                    ...
                    if cq_candidates:
                        ...
                        for cq in cq_candidates:
                            ...
                            if c4.button("선택", key=f"cq_{cq['단지명']}"):
                                st.session_state._pending_loc = _extract_loc(cq['주소'])
                                st.session_state._pending_apt = cq['단지명']
