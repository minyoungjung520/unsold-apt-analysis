...
def fetch_kakao_apt_name(address):
    """주소로 카카오 로컬 API 검색 → 아파트명 반환"""
    ...
    if docs:
        ...
        if building:
            return building
    # 건물명 없으면 키워드 검색으로 재시도
    url2 = "https://dapi.kakao.com/v2/local/search/keyword.json"
    res2 = requests.get(url2, headers=headers, params={"query": address, "category_group_code": "AP"}, timeout=10)
    if res2.status_code == 200:
        docs2 = res2.json().get("documents", [])
        if docs2:
            return docs2[0].get("place_name", "")
    return None
...
# 주소로 아파트명 자동 검색
with st.expander("주소로 아파트명 자동 검색 (번지 입력)"):
    addr_col1, addr_col2 = st.columns([4, 1])
    with addr_col1:
        address_input = st.text_input("상세 주소 입력", placeholder="예: 대구 수성구 범어동 1234-5", label_visibility="collapsed")
    with addr_col2:
        addr_btn = st.button("아파트명 찾기", use_container_width=True)
    if addr_btn and address_input:
        with st.spinner("카카오 주소 검색 중..."):
            found_name = fetch_kakao_apt_name(address_input)
        if found_name:
            st.success(f"찾은 아파트명: **{found_name}**")
            st.info("아래 아파트명 입력란에 복사해서 사용하세요.")
        else:
            st.warning("아파트명을 자동으로 찾지 못했습니다. 직접 입력해 주세요.")
