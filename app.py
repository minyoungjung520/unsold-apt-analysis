...
with st.expander("주소 또는 단지명으로 아파트명 검색"):
    ...
    with addr_col1:
        address_input = st.text_input("주소 또는 단지명 입력", placeholder="예: 대구 수성구 범어동 1000-1  또는  대구 수성구 범어자이", label_visibility="collapsed")
    with addr_col2:
    ...
    if addr_btn and address_input:
        ...
        if candidates:
            st.success(f"검색 결과 {len(candidates)}건 — 아래에서 선택 후 아파트명 입력란에 복사하세요.")
            for c in candidates:
