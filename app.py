...
# 세션스테이트 초기화
for _k, _v in [("kakao_candidates",[]), ("cq_selected_info", None), ("cq_selected_loc", "")]:
...
# 청약홈 선택 데이터가 있으면 해당 데이터로 바로 분석
cq_run = bool(st.session_state.cq_selected_info)

if search_btn or cq_run:
    # 청약홈 직접 선택 시 location/apt_name 오버라이드
    if cq_run:
