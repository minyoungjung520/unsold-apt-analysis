...
# 카카오/청약홈 선택값으로 드롭다운 기본값 결정
_last_parts = st.session_state.last_loc.split() if st.session_state.last_loc else []
_sido_list = list(SIDO_SIGUNGU.keys())
_def_sido = _last_parts[0] if _last_parts and _last_parts[0] in SIDO_SIGUNGU else "대구"
_def_dong = _last_parts[2] if len(_last_parts) > 2 else ""

colA, colB, colC, col2, col3, col4, col5 = st.columns([1, 1.2, 1.1, 1.8, 1, 1, 1])
with colA:
    sel_sido = st.selectbox("시/도", _sido_list, index=_sido_list.index(_def_sido))
with colB:
    _sgg_list = SIDO_SIGUNGU[sel_sido]
    _def_sgg_idx = _sgg_list.index(_last_parts[1]) if len(_last_parts) > 1 and _last_parts[1] in _sgg_list else 0
    sel_sigungu = st.selectbox("시/군/구", _sgg_list, index=_def_sgg_idx)
with colC:
    sel_dong = st.text_input("읍/면/동", value=_def_dong, placeholder="예: 범어동")
location = f"{sel_sido} {sel_sigungu} {sel_dong}".strip()
with col2:
